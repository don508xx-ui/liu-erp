...
def _flows(db):
    ...
    for name, biz, nodes in flows:
        if not db.query(FlowDefinition).filter(FlowDefinition.biz_type == biz).first():
            db.add(FlowDefinition(name=name, biz_type=biz, nodes=nodes, status="ACTIVE", version=1))

...
清空策略: 只清空事务数据,保留基础档案(客户/供应商/物料/科目/账户等)
"""
import sys, os
...
# 资金账户ID快捷引用
ACC_ID = {
    'JX_BANK': accounts.get('JX-BANK'),
    'DG_BANK': accounts.get('DG-BANK'),
    'ACCEPTANCE': accounts.get('ACCEPTANCE'),
    'CASH': accounts.get('CASH'),
}

# 客户按公司分流: 大件/高价值走机械公司, 小件/标准件走加工厂
COMPANY1_CUSTOMERS = [1, 5, 7, 8]  # 机械公司客户
COMPANY2_CUSTOMERS = [2, 3, 6]     # 加工厂客户

# 常用工艺/规格池
CRAFTS = ["超音速火焰喷涂", "等离子喷涂", "氧乙炔火焰陶瓷棒", "碳化钨防粘涂层", "碳纤维防粘涂层"]
SPECS = [
    ("活塞杆 Φ80×1200", "BY_AREA", 350, "m²"),
    ("辊轴 Φ150×2000", "BY_AREA", 420, "m²"),
    ("耐磨衬板 500×400", "BY_PIECE", 180, "件"),
    ("导轮 Φ200", "BY_PIECE", 95, "件"),
    ("密封环 Φ300×50", "BY_PIECE", 65, "件"),
    ("气缸套 Φ120×300", "BY_PIECE", 120, "件"),
    ("刮刀片 600×80", "BY_PIECE", 85, "件"),
    ("搅拌桨叶 400×300", "BY_PIECE", 250, "件"),
    ("喷嘴 Φ50×100", "BY_PIECE", 55, "件"),
    ("链轮 Φ250", "BY_PIECE", 110, "件"),
]

# 供应商池
SUPPLIER_NAMES = ["立邦涂料(上海)有限公司", "阿克苏诺贝尔粉末涂料", "上海某前处理材料商行", "某委外电镀加工厂"]

# 行业: 客户id→行业名
CUST_INDUSTRY = {1: "汽配", 2: "家电", 3: "五金", 4: "化工", 5: "机械", 6: "电子", 7: "机械", 8: "汽配"}

# ============================================================
# 第3步: 创建订单→工单→完工→应收的完整链条
# ============================================================
print("\n第3步: 创建订单→工单→应收单据")

# 订单数据设计: 8个月(2026-01~2026-08), 每个客户每月1-2个订单
# 业务逻辑: 客户下单→工厂接单→排产→完工→发货→开应收单→收款

MONTHS = [(2026, m) for m in range(1, 9)]  # 1~8月
order_no = 0
total_orders = 0
...
for mi, (year, month) in enumerate(MONTHS):
    ...
    for cid in COMPANY1_CUSTOMERS + COMPANY2_CUSTOMERS:
        # 机械公司客户每月1-2单, 加工厂客户每月1单
        n_orders = 2 if cid in COMPANY1_CUSTOMERS else 1
        if mi == 0 and cid in COMPANY2_CUSTOMERS:
            n_orders = 1
        for oi in range(n_orders):
            ...
            for ii in range(n_items):
                ...
                qty = round(5 + (order_no * 7 + ii * 13) % 45, 0)  # 5~50件
                if price_type == "BY_AREA":
                    qty = round(qty * 0.5, 2)
                amt = round(qty * price * base, 2)
                order_amt += amt
                items_data.append({
                    "part_name": spec.split()[0],
                    "part_spec": spec,
                    "price_type": price_type,
                    "quantity": qty,
                    "unit": unit,
                    "unit_price": round(price * base, 2),
                    "amount": amt,
            ...
            # 创建订单
            o = Order(
                order_no=f"SO-{year}{month:02d}-{order_no:03d}",
                customer_id=cid, company_id=company_id,
                status="EFFECTIVE", total_amount=order_amt,
                signed_at=order_date, effective_at=order_date,
                billing_type="SPECIAL_VAT",
                sales_user_id=2 if cid in COMPANY1_CUSTOMERS else 3,
            )
            db.add(o); db.flush()
            for ii, item in enumerate(items_data):
                db.add(OrderItem(
                    order_id=o.id, seq=ii+1,
                    part_name=item["part_name"], part_spec=item["part_spec"],
                    price_type=item["price_type"], quantity=item["quantity"],
                    unit=item["unit"], unit_price=item["unit_price"],
                    amount=item["amount"], craft_type=item["craft_type"],
                    material_mode="SELF",
                ))
            # 创建工单(每个订单1个工单, 按主规格)
            main_item = items_data[0]
            wo = WorkOrder(
                work_order_no=f"WO-{year}{month:02d}-{order_no:03d}",
                order_id=o.id, customer_id=cid,
                customer_name=customers[cid].name,
                product_spec=main_item["part_spec"],
                process=main_item["craft_type"],
                workshop="A车间" if company_id == 1 else "B车间",
                status="COMPLETED",
                plan_qty=main_item["quantity"],
                actual_qty=main_item["quantity"],
            ...
            scrap = round(float(main_item["quantity"]) * 0.03, 3)
            cp = Completion(
                completion_no=f"CP-{year}{month:02d}-{order_no:03d}",
                work_order_id=wo.id, status="CONFIRMED",
                finished_qty=main_item["quantity"], qualified_qty=qualified,
                scrap_qty=scrap, rework_qty=round(float(main_item["quantity"]) * 0.02, 3),
            ...
            db.add(CompletionItem(
                completion_id=cp.id, item_id=1, item_name="碳化钨粉末",
            ...
            # 应收单据(金额=订单金额)
            receivable = round(order_amt * 0.9, 2)  # 90%金额开应收(10%质保金)
            ...
            fd = FinanceDoc(
                doc_no=f"AR-{year}{month:02d}-{order_no:03d}",
                doc_type="RECEIVABLE", status=status,
                related_type="ORDER", related_id=o.id,
                counterparty_type="CUSTOMER", counterparty_id=cid,
                counterparty_name=customers[cid].name,
                amount=order_amt, settled_amount=round(settled, 2),
                company_id=company_id, billing_type="SPECIAL_VAT",
                account_date=order_date + timedelta(days=20),
            ...
            if settled > 0:
                ...
                rc = FinanceDoc(
                    doc_no=f"RC-{year}{month:02d}-{receipt_no:03d}",
                    doc_type="RECEIPT", status="SETTLED",
                    related_type="ORDER", related_id=o.id,
                    counterparty_type="CUSTOMER", counterparty_id=cid,
                    counterparty_name=customers[cid].name,
                    amount=round(settled, 2), settled_amount=round(settled, 2),
                    company_id=company_id,
                    account_date=settled_at or (order_date + timedelta(days=45)),
                    settled_at=settled_at,
                    source_event="payment_received",
                )
                db.add(rc); db.flush()
                # 资金流水: 收款→资金流入
                # 机械公司客户→机械公账, 加工厂客户→加工厂公账
                fa_id = ACC_ID['JX_BANK'] if company_id == 1 else ACC_ID['DG_BANK']
                db.add(FundFlow(
                    fund_account_id=fa_id, direction="IN",
...
db.commit()
print(f"  创建订单: {total_orders}个, 应收总额: {total_receivable:.2f}元")

# ============================================================
# 第4步: 供应商采购→应付→付款→资金流出
# ============================================================
print("\n第4步: 采购应付→付款→资金流出")
...
for mi, (year, month) in enumerate(MONTHS):
    ...
    for company_id in [1, 2]:
        ...
        for pi in range(n_po):
            ...
            # 创建采购单
            p = Purchase(
                po_no=f"PO-{year}{month:02d}-{po_no:03d}",
                supplier_id=sid, status="RECEIVED",
                total_amount=po_amt,
            ...
            if settled > 0:
                pmt_no = po_no * 10 + 2
                pmt = FinanceDoc(
                    doc_no=f"PY-{year}{month:02d}-{pmt_no:03d}",
                    doc_type="PAYMENT", status="SETTLED",
                    related_type="PURCHASE", related_id=p.id,
                    counterparty_type="SUPPLIER", counterparty_id=sid,
                    counterparty_name=supplier_name,
                    amount=round(settled, 2), settled_amount=round(settled, 2),
                ...
                fa_id = ACC_ID['JX_BANK'] if company_id == 1 else ACC_ID['DG_BANK']
                db.add(FundFlow(
                    fund_account_id=fa_id, direction="OUT", amount=round(settled, 2),
                    expense_category="耗材",
...
for mi, (year, month) in enumerate(MONTHS):
    ...
    for company_id in [1, 2]:
        ...
        pr_no += 1
        pr = PayrollRun(
            run_no=f"PR-{year}{month:02d}-{company_id}",
            period=f"{year}-{month:02d}",
            total_amount=wage_amt, status="PAID",
            items=[{"employee_id": 1, "name": "员工A", "department": "生产", "base_salary": wage_amt * 0.4,
                    "bonus": wage_amt * 0.1, "net": wage_amt * 0.5, "bank_amount": wage_amt * 0.45, "cash_amount": wage_amt * 0.05}],
            paid_at=pay_date,
        )
        db.add(pr); db.flush()
        # 工资付款单
        pmt = FinanceDoc(
            doc_no=f"PY-W{year}{month:02d}-{company_id:03d}",
            doc_type="PAYROLL", status="SETTLED",
            related_type="PAYROLL", related_id=pr.id,
            counterparty_type="EMPLOYEE", counterparty_name="全体员工",
            amount=wage_amt, settled_amount=wage_amt,
            company_id=company_id,
            account_date=pay_date, settled_at=pay_date,
            source_event="payroll",
        )
        db.add(pmt); db.flush()
        # 双向关联: 工资单 -> 付款单
        ...
        fa_id = ACC_ID['JX_BANK'] if company_id == 1 else ACC_ID['DG_BANK']
        db.add(FundFlow(
            fund_account_id=fa_id, direction="OUT", amount=wage_amt,
            expense_category="工资",
            counterparty="全体员工",
            occur_date=pay_date,
        ...
        if ACC_ID['CASH']:
            db.add(FundFlow(
                fund_account_id=ACC_ID['CASH'], direction="OUT", amount=cash_amt,
                expense_category="工资",
                counterparty="全体员工",
                occur_date=pay_date,
...
# ============================================================
# 第6步: 经营费用(直接资金流出,带expense_category)
# ============================================================
print("\n第6步: 经营费用(气体/食堂/佣金/提成/融资成本/其他)")

# 费用按天均匀分布, 确保看板支出结构有数据
expense_plan = {
    "气体": [(3000, 8000), (1500, 4000)],       # 机械/加工厂
    "食堂": [(1500, 3000), (800, 1800)],
    "佣金": [(5000, 15000), (2000, 6000)],
    "提成": [(4000, 12000), (1500, 4000)],
    "融资成本": [(800, 3500), (500, 2500)],
    "其他": [(1000, 5000), (500, 3000)],
}
        ...
        for company_id in [1, 2]:
            ...
            for cat in cats:
                ...
                if cat == "食堂" and rng.random() < 0.1 and ACC_ID['CASH']:
                    fa_id = ACC_ID['CASH']
                db.add(FundFlow(
                    fund_account_id=fa_id, direction="OUT", amount=amt,
...
if ACC_ID['CASH']:
    for mi, (year, month) in enumerate(MONTHS):
        for week in range(4):
            ...
            db.add(FundFlow(
                fund_account_id=ACC_ID['CASH'], direction="IN", amount=amt,
                counterparty="零星客户",
                occur_date=datetime(year, month, day),
                summary="现金收款(零星)",
                source_type="RECEIPT",
            ))
            cash_income += 1
...
acceptance_flows = 0
for mi, (year, month) in enumerate(MONTHS[2:], 2):  # 3月开始
    base = 0.7 + 0.5 * mi / (len(MONTHS) - 1)
    if mi % 2 == 0:  # 隔月有承兑
        ...
        if ACC_ID['ACCEPTANCE']:
            db.add(FundFlow(
                fund_account_id=ACC_ID['ACCEPTANCE'], direction="IN", amount=amt,
                counterparty="上海汽车配件有限公司" if mi % 2 == 0 else "无锡机械制造有限公司",
                occur_date=exp_date,
                summary=f"收承兑汇票 {amt:.2f}元",
            ...
            if due_date < now:
                db.add(FundFlow(
                    fund_account_id=ACC_ID['ACCEPTANCE'], direction="OUT", amount=amt,
                    counterparty="银行托收",
                    occur_date=due_date,
                    summary=f"承兑到期托收 {amt:.2f}元",
                    source_type="ACCEPTANCE_SETTLE",
                ))
                db.add(FundFlow(
                    fund_account_id=ACC_ID['JX_BANK'], direction="IN", amount=amt,
                    counterparty="承兑到期托收",
                    occur_date=due_date,
                    summary=f"承兑到期入账 {amt:.2f}元",
                    source_type="ACCEPTANCE_SETTLE",
                ))
                acceptance_flows += 2
